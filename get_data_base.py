...
def main():
    # Load config
    try:
        with open('config/fxswaps.json', 'r') as f:
            data = json.load(f)
    ...
    base_ids = [int(k) for k, v in data.items() if v.get('chain_name') == 'base']
    ...
    for idx in base_ids:
        ...
        try:
            subprocess.run(["python3", "scripts/fill_missing_fxswap_data.py", "--index", str(idx)], check=True)
