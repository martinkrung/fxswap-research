...
parser = argparse.ArgumentParser(
    description="Analyze volatility and rebalancing costs for fxswap pools"
)
parser.add_argument(
...
def sanitize_path_component(component):
    ...
    if not sanitized:
        raise ValueError(
            f"Path component '{component}' became empty after sanitization"
        )
    return sanitized
...
for _block_number, block_data in sorted(data.items(), key=lambda x: int(x[0])):
    ...
    if (
        ...
        and "price_scale" in block_data
        and "price_oracle" in block_data
    ):
        price_data.append(
            {
                "timestamp": timestamp,
    ...
    if "donation_shares" in block_data and "last_donation_release_ts" in block_data:
        ...
        if (
            prev_release_ts is not None
            and release_ts != prev_release_ts
            and release_ts > 1000000000
        ):
            is_rebalance = True
...
donation_df = (
    pd.DataFrame(donation_data)
    .drop_duplicates(subset=["timestamp"])
    .sort_values("timestamp")
)
supply_df = (
    pd.DataFrame(supply_data)
    .drop_duplicates(subset=["timestamp"])
    .sort_values("timestamp")
)
...
price_df["log_returns"] = np.log(
    price_df["last_price"] / price_df["last_price"].shift(1)
)

# 2. Calculate price range for each observation
price_df["price_range"] = (
    price_df["last_price"].rolling(window="1h").max()
    - price_df["last_price"].rolling(window="1h").min()
)
price_df["price_range_pct"] = (price_df["price_range"] / price_df["last_price"]) * 100

# 3. Rolling volatility metrics (7-day window)
# Standard deviation of returns (annualized)
price_df["volatility_std"] = price_df["returns"].rolling(
...
price_df["volatility_mad"] = price_df["returns"].abs().rolling(
    window=VOLATILITY_WINDOW
).mean() * np.sqrt(365)
...
price_df["volatility_range"] = (price_df["high_7d"] - price_df["low_7d"]) / price_df[
    "last_price"
]

# Coefficient of variation (normalized volatility)
price_df["cv"] = (
    price_df["last_price"].rolling(window=VOLATILITY_WINDOW).std()
...
price_df["delta_abs_ma"] = (
    price_df["delta_price_pct"].abs().rolling(window=VOLATILITY_WINDOW).mean()
)
...
donation_df["cost_usd"] = donation_df.apply(
    ...
    if pd.notna(row["totalSupply"])
    and row["totalSupply"] > 0
    and pd.notna(row["tvl_usd"])
    and row["shares_used"] > 0
    else 0,
    axis=1,
)
...
donation_df["cost_usd_7d"] = (
    donation_df["cost_usd"].rolling(window=VOLATILITY_WINDOW).sum()
)
donation_df["cost_pct_tvl_7d"] = (
    donation_df["cost_pct_tvl"].rolling(window=VOLATILITY_WINDOW).sum()
)

# Rebalancing frequency (count of events in 7-day window)
donation_df["rebalance_count_7d"] = (
    donation_df["is_rebalance"].rolling(window=VOLATILITY_WINDOW).sum()
)

donation_df = donation_df.reset_index()

# Merge price and donation data for correlation analysis
merged_df = pd.merge(price_df, donation_df, on="timestamp", how="outer").sort_values(
    "timestamp"
)
...
if len(correlation_data) > 10:
    ...
    print("\n=== Key Correlations ===")
    if (
        "volatility_std" in corr_matrix.columns
        and "cost_pct_tvl_7d" in corr_matrix.columns
    ):
        print(
            f"Volatility (Std) vs Cost/TVL: {corr_matrix.loc['volatility_std', 'cost_pct_tvl_7d']:.3f}"
        )
    if (
        "volatility_std" in corr_matrix.columns
        and "rebalance_count_7d" in corr_matrix.columns
    ):
        print(
            f"Volatility (Std) vs Rebalance Frequency: {corr_matrix.loc['volatility_std', 'rebalance_count_7d']:.3f}"
        )
    if (
        "delta_abs_ma" in corr_matrix.columns
        and "cost_pct_tvl_7d" in corr_matrix.columns
    ):
        print(
            f"Price Delta (abs) vs Cost/TVL: {corr_matrix.loc['delta_abs_ma', 'cost_pct_tvl_7d']:.3f}"
        )
else:
...
if not price_df.empty:
    ...
    print(
        f"  Price Range: ${price_df['last_price'].min():.4f} - ${price_df['last_price'].max():.4f}"
    )
    print(f"  Mean 7D Volatility (Std): {price_df['volatility_std'].mean():.2%}")
    print(
        f"  Mean 7D Volatility (Realized): {price_df['volatility_realized'].mean():.2%}"
    )
...
if not donation_df.empty:
    ...
    if "tvl_usd" in donation_df.columns:
        ...
        if mean_tvl > 0:
            ...
            print(
                f"  Total Cost as % of Mean TVL: {(total_cost / mean_tvl) * 100:.4f}%"
            )
...
fig1, axes1 = plt.subplots(
    8, 1, figsize=(figure_width_cm / 2.54, 50 / 2.54), sharex=True
)
...
if not price_df.empty:
    ...
    if not vol_data.empty:
        ax.plot(
            vol_data["timestamp"],
            vol_data["volatility_std"] * 100,
            color=RED,
            label="Std Dev (annualized)",
            linestyle="-",
            linewidth=1,
            alpha=0.8,
        )
    ...
    if not vol_data.empty:
        ax.plot(
            vol_data["timestamp"],
            vol_data["volatility_realized"] * 100,
            color=ORANGE,
            label="Realized Vol",
            linestyle="-",
            linewidth=1,
...
if not price_df.empty:
    ...
    if not delta_data.empty:
        ax.plot(
            delta_data["timestamp"],
            delta_data["delta_abs_ma"],
            color="magenta",
            label="|Delta| 7D MA",
            linestyle="-",
            linewidth=1.5,
...
ax.set_title(
    "Absolute Price Delta (7-Day Moving Average)", fontsize=12, fontweight="bold"
)
ax.legend(fontsize=8, loc="upper right")
ax.grid(True, alpha=0.3)
...
if not supply_df.empty:
    ...
    ax.fill_between(
        supply_df["timestamp"], 0, supply_df["tvl_usd"], color=GREEN, alpha=0.2
    )
...
ax = axes1[5]
rebalance_events = donation_df[donation_df["is_rebalance"]]
if not rebalance_events.empty:
    for _idx, row in rebalance_events.iterrows():
        ax.axvline(x=row["timestamp"], color=RED, linestyle="-", linewidth=1, alpha=0.7)
ax.set_ylabel("Events", fontsize=10)
ax.set_title(
    f"Rebalancing Events (n={len(rebalance_events)})", fontsize=12, fontweight="bold"
)
ax.set_ylim(0, 1)
ax.set_yticks([])
ax.grid(True, alpha=0.3)
...
if not donation_df.empty:
    ...
    if not cost_7d_data.empty:
        ax_twin = ax.twinx()
        ax_twin.plot(
            cost_7d_data["timestamp"],
            cost_7d_data["cost_usd_7d"],
            color=RED,
            label="7D Cumulative",
            linestyle="-",
            linewidth=1.5,
...
if not donation_df.empty:
    ...
    if not cost_pct_data.empty:
        bar_width = timedelta(hours=12)
        ax.bar(
            cost_pct_data["timestamp"],
            cost_pct_data["cost_pct_tvl"],
            width=bar_width,
            color=PURPLE,
            label="Cost % TVL",
            alpha=0.7,
    ...
    if not cost_7d_pct_data.empty:
        ax_twin = ax.twinx()
        ax_twin.plot(
            cost_7d_pct_data["timestamp"],
            cost_7d_pct_data["cost_pct_tvl_7d"],
            color="darkred",
            label="7D Cumulative",
            linestyle="-",
            linewidth=1.5,
...
if corr_matrix is not None and len(correlation_data) > 10:
    ...
    if len(valid_data) > 5:
        ax.scatter(
            valid_data["volatility_std"] * 100,
            valid_data["cost_pct_tvl_7d"],
            color=BLUE,
            alpha=0.5,
            s=20,
        )
        # Add trend line
        z = np.polyfit(
            valid_data["volatility_std"] * 100, valid_data["cost_pct_tvl_7d"], 1
        )
        p = np.poly1d(z)
        x_trend = np.linspace(
            valid_data["volatility_std"].min() * 100,
            valid_data["volatility_std"].max() * 100,
            100,
        )
        ...
        r_value = stats.pearsonr(
            valid_data["volatility_std"], valid_data["cost_pct_tvl_7d"]
        )[0]
        ax.text(
            0.05,
            0.95,
    ...
    ax = axes2[0, 1]
    valid_data = correlation_data.dropna(
        subset=["volatility_std", "rebalance_count_7d"]
    )
    if len(valid_data) > 5:
        ax.scatter(
            valid_data["volatility_std"] * 100,
            valid_data["rebalance_count_7d"],
            color=GREEN,
            alpha=0.5,
            s=20,
        )
        z = np.polyfit(
            valid_data["volatility_std"] * 100, valid_data["rebalance_count_7d"], 1
        )
        p = np.poly1d(z)
        x_trend = np.linspace(
            valid_data["volatility_std"].min() * 100,
        ...
        r_value = stats.pearsonr(
            valid_data["volatility_std"], valid_data["rebalance_count_7d"]
        )[0]
        ax.text(
            0.05,
            0.95,
    ...
    if len(valid_data) > 5:
        ax.scatter(
            valid_data["volatility_range"] * 100,
            valid_data["cost_pct_tvl_7d"],
            color=ORANGE,
            alpha=0.5,
            s=20,
        ...
        z = np.polyfit(
            valid_data["volatility_range"] * 100, valid_data["cost_pct_tvl_7d"], 1
        )
        p = np.poly1d(z)
        x_trend = np.linspace(
            valid_data["volatility_range"].min() * 100,
            valid_data["volatility_range"].max() * 100,
            100,
        )
        ax.plot(x_trend, p(x_trend), color=RED, linestyle="--", linewidth=2)

        r_value = stats.pearsonr(
            valid_data["volatility_range"], valid_data["cost_pct_tvl_7d"]
        )[0]
        ax.text(
            0.05,
            0.95,
    ...
    if len(valid_data) > 5:
        ...
        p = np.poly1d(z)
        x_trend = np.linspace(
            valid_data["delta_abs_ma"].min(), valid_data["delta_abs_ma"].max(), 100
        )
        ax.plot(x_trend, p(x_trend), color=RED, linestyle="--", linewidth=2)

        r_value = stats.pearsonr(
            valid_data["delta_abs_ma"], valid_data["cost_pct_tvl_7d"]
        )[0]
        ax.text(
            0.05,
            0.95,
    ...
    ax = axes2[1, 1]
    valid_data = correlation_data.dropna(
        subset=["volatility_realized", "cost_pct_tvl_7d"]
    )
    if len(valid_data) > 5:
        ax.scatter(
            valid_data["volatility_realized"] * 100,
            valid_data["cost_pct_tvl_7d"],
            color="cyan",
            alpha=0.5,
            s=20,
        ...
        z = np.polyfit(
            valid_data["volatility_realized"] * 100, valid_data["cost_pct_tvl_7d"], 1
        )
        p = np.poly1d(z)
        x_trend = np.linspace(
            valid_data["volatility_realized"].min() * 100,
            valid_data["volatility_realized"].max() * 100,
            100,
        )
        ax.plot(x_trend, p(x_trend), color=RED, linestyle="--", linewidth=2)

        r_value = stats.pearsonr(
            valid_data["volatility_realized"], valid_data["cost_pct_tvl_7d"]
        )[0]
        ax.text(
            0.05,
            0.95,
...
if not price_df.empty:
    ...
    if len(returns_clean) > 0:
        ...
        ax.set_title(
            f"Returns Distribution (μ={returns_clean.mean() * 100:.3f}%, σ={returns_clean.std() * 100:.3f}%)",
            fontsize=11,
            fontweight="bold",
        )
        ax.grid(True, alpha=0.3)
...
if not price_df.empty:
    ...
    if len(vol_clean) > 0:
        ...
        ax.set_ylabel("Frequency", fontsize=10)
        ax.set_title(
            f"Volatility Distribution (μ={vol_clean.mean() * 100:.2f}%)",
            fontsize=11,
            fontweight="bold",
        )
        ax.grid(True, alpha=0.3)
...
if not donation_df.empty:
    ...
    if len(cost_clean) > 0:
        ...
        ax.set_ylabel("Frequency", fontsize=10)
        ax.set_title(
            f"Rebalancing Cost Distribution (μ={cost_clean.mean():.4f}%)",
            fontsize=11,
            fontweight="bold",
        )
        ax.grid(True, alpha=0.3)
...
if not price_df.empty:
    ...
    price_df_copy["week"] = (
        pd.to_datetime(price_df_copy["timestamp"]).dt.isocalendar().week
    )
    weekly_vol = price_df_copy.groupby("week")["volatility_std"].apply(list)
    if len(weekly_vol) > 0:
        ax.boxplot(
            [v for v in weekly_vol.values if len(v) > 0], tick_labels=weekly_vol.index
        )
...
if not merged_df.empty:
    daily_agg = (
        ...
        .resample("1D")
        .agg({"volatility_std": "mean", "cost_pct_tvl": "sum"})
        .reset_index()
    )

    ax.bar(
        daily_agg["timestamp"],
        daily_agg["cost_pct_tvl"],
        color=PURPLE,
        alpha=0.6,
        label="Daily Cost % TVL",
    )
    ...
    ax2.plot(
        daily_agg["timestamp"],
        daily_agg["volatility_std"] * 100,
        color=RED,
        linewidth=2,
        label="Daily Avg Volatility",
    )
    ...
    ax.set_title(
        "Daily Aggregates: Cost and Volatility", fontsize=11, fontweight="bold"
    )
    ax.grid(True, alpha=0.3)
    ax.xaxis.set_major_formatter(mdates.DateFormatter("%m/%d"))
    plt.setp(ax.xaxis.get_majorticklabels(), rotation=45)
