...
fxswap_contract = boa.from_etherscan(
    fxswap_address, name="fxswap", chain_id=int(XSCAN_CHAIN_ID), api_key=XSCAN_API_KEY
)
...
min_mint_amount = fxswap_contract.calc_token_amount(
...
if not SIM:
    refuel_tx = fxswap_contract.add_liquidity(
        **params, sender=signer.address, gas=300_000
    )
    print(refuel_tx)
else:
