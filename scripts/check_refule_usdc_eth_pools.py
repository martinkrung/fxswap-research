...
def get_block_timestamp_boa(block_number, rpc_url=None):
    ...
    try:
        ...
        try:
            ...
            timestamp = getattr(
                block_header, "timestamp", getattr(block_header, "epoch", None)
            )
...
def get_csv_file(pool_address, pool_name=None):
    ...
    if pool_identifier not in csv_files:
        ...
        if pool_name:
            ...
            safe_name = (
                pool_identifier[-8:] if len(pool_identifier) > 8 else pool_identifier
            )
        else:
            safe_name = "general"

        csv_filename = os.path.join(
            log_dir, f"check_refule_usdc_eth_pools_{safe_name}.csv"
        )
...
for fxswap_address in fxswap_addresses:
    lp_price = boa.env.raw_call(
        to=fxswap_address, data="0x54f0f7d5", block_identifier=37788206
    )
    print(f"lp_price; {lp_price}")

    fxswap_contract = boa.from_etherscan(
        fxswap_address, name="fxswap", chain_id=8453, api_key=XSCAN_API_KEY
    )
    ...
    for fxswap_address in fxswap_addresses:
        ...
        if False:
            token0_contract = boa.from_etherscan(
                token0_address, name="token0", chain_id=8453, api_key=XSCAN_API_KEY
            )
            ...
            token1_contract = boa.from_etherscan(
                token1_address, name="token1", chain_id=8453, api_key=XSCAN_API_KEY
            )
        ...
        print(
            f"donation_shares_max_ratio(); {fxswap_contract.donation_shares_max_ratio()}"
        )
        ...
        print(
            f"last_donation_release_ts; {datetime.fromtimestamp(last_donation_release_ts)}"
        )
        ...
        print(
            f"donation_protection_expiry_ts(); {fxswap_contract.donation_protection_expiry_ts()}"
        )
        print(
            f"donation_protection_period(); {fxswap_contract.donation_protection_period()}"
        )
        ...
        calculate_lp_share = fxswap_contract.calc_token_amount(
            [balance_0, balance_1], True
        )
        ...
        print(
            f"last_donation_release_ts(); {fxswap_contract.last_donation_release_ts()}"
        )
        print(
            f"donation_protection_expiry_ts(); {fxswap_contract.donation_protection_expiry_ts()}"
        )
        ...
        print(
            f"user supply is the total supply minus the donation shares; {fxswap_contract.totalSupply() - fxswap_contract.donation_shares()}"
        )
        ...
        delta_price_last_to_oracle = (
            fxswap_contract.last_prices() - fxswap_contract.price_oracle()
        )
        print(
            f"delta_price_last_to_oracle() in USD; {delta_price_last_to_oracle / 10**18}"
        )

        delta_price_last_to_oracle_percent = (
            delta_price_last_to_oracle / fxswap_contract.price_oracle()
        ) * 100
        print(
            f"delta_price_last_to_oracle() in %; {delta_price_last_to_oracle_percent:.6f}%"
        )

        delta_price_last_to_scale = (
            fxswap_contract.last_prices() - fxswap_contract.price_scale()
        )
        print(
            f"delta_price_last_to_scale() in USD; {delta_price_last_to_scale / 10**18}"
        )
        delta_price_last_to_scale_percent = (
            delta_price_last_to_scale / fxswap_contract.price_scale()
        ) * 100
        print(
            f"delta_price_last_to_scale() in %; {delta_price_last_to_scale_percent:.6f}%"
        )

        delta_price_oracle_to_scale = (
            fxswap_contract.price_oracle() - fxswap_contract.price_scale()
        )
        print(
            f"delta_price_oracle_to_scale() in USD; {delta_price_oracle_to_scale / 10**18}"
        )
        delta_price_oracle_to_scale_percent = (
            delta_price_oracle_to_scale / fxswap_contract.price_scale()
        ) * 100
        print(
            f"delta_price_oracle_to_scale() in %; {delta_price_oracle_to_scale_percent:.6f}%"
        )
        ...
        print(
            f"packed_rebalancing_params(); {fxswap_contract.packed_rebalancing_params()}"
        )
        print(f"donation_duration(); {fxswap_contract.donation_duration()}")
        print(
            f"donation_protection_lp_threshold(); {fxswap_contract.donation_protection_lp_threshold()}"
        )
        time.sleep(2)
