...
# Get token contracts
coin0 = boa.from_etherscan(
    coin0_address, name="USDC", chain_id=8453, api_key=XSCAN_API_KEY
)
coin1 = boa.from_etherscan(
    coin1_address, name="WETH", chain_id=8453, api_key=XSCAN_API_KEY
)
...
if initial_usdc_balance is None or initial_eth_balance is None:
    ...
    if initial_usdc_balance is None:
        print("  USDC balance unavailable")
    if initial_eth_balance is None:
        print("  WETH balance unavailable")
    sys.exit(1)
...
if (
    usdc_allowance < SWAP_AMOUNT_USDC * NUM_SWAPS * 2
):  # Need enough for all swaps both ways
    print("\nApproving USDC...")
    ...
    try:
        ...
        if (
            "'NoneType' object is not subscriptable" in str(e)
            or "fork" in str(e).lower()
        ):
            print(
                "Approval transaction mined successfully (fork reset warning ignored)"
            )
            print(f"Approved {approve_amount / 10**USDC_DECIMALS:.2f} USDC")
        else:
            raise
...
for i in range(NUM_SWAPS):
    ...
    try:
        ...
        if dy_expected is None:
            raise Exception("Could not get dy via titanoboa or web3") from e
    ...
    try:
        ...
        if (
            ...
            print("  Transaction mined successfully (fork reset warning ignored)")
        else:
            raise
    ...
    if eth_balance_after is None:
        print("  Warning: Could not get ETH balance, using expected value")
    ...
    time.sleep(1)  # Small delay before API call
    usdc_balance_before = get_token_balance(
        coin0_address, signer.address, USDC_DECIMALS
    )
    ...
    try:
        ...
        try:
            ...
            if dy_expected_usdc is None:
                raise Exception("Could not get dy via titanoboa or web3") from e2
    ...
    if usdc_balance_after is None:
        print("  Warning: Could not get USDC balance, using expected value")
...
if final_usdc_balance is None or final_eth_balance is None:
    ...
    print("\nNet change:")
    print(
        f"  USDC: {(final_usdc_balance - initial_usdc_balance) / 10**USDC_DECIMALS:.2f}"
    )
