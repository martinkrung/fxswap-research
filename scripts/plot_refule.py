...
for _block_number, block_data in sorted(data.items(), key=lambda x: int(x[0])):
    ...
    if "last_prices" in block_data:
        last_prices_data.append(
            {"timestamp": timestamp, "last_price": block_data["last_prices"]["value"]}
        )

    # Extract price_scale
    if "price_scale" in block_data:
        price_scale_data.append(
            {"timestamp": timestamp, "price_scale": block_data["price_scale"]["value"]}
        )
    ...
    if "totalSupply" in block_data:
        total_supply_data.append(
            {"timestamp": timestamp, "totalSupply": block_data["totalSupply"]["value"]}
        )
    ...
    if balance_0 is not None or balance_1 is not None:
        balance_data.append(
            {"timestamp": timestamp, "balance_0": balance_0, "balance_1": balance_1}
        )
    ...
    if "last_donation_release_ts" in block_data:
        ...
        if release_ts > 1000000000:  # Valid unix timestamp (after 2001)
            ...
            donation_releases.append(
                {"timestamp": timestamp, "release_time": release_time}
            )
    ...
    if (
        "donation_shares" in block_data
        and "totalSupply" in block_data
        and "balances(0)" in block_data
        and "balances(1)" in block_data
        and "last_prices" in block_data
    ):
        donation_shares = block_data["donation_shares"]["value"]
        ...
        if (
            prev_donation_shares is not None
            and prev_total_supply is not None
            and prev_total_supply > 0
            and total_supply > 0
        ):
            # Calculate growth rate of total_supply
            total_supply_growth_rate = (
            ...
            expected_donation_shares = prev_donation_shares * (
                1 + total_supply_growth_rate
            )

            # Calculate actual growth rate of donation_shares
            if prev_donation_shares > 0:
                donation_shares_growth_rate = (
                    donation_shares - prev_donation_shares
                ) / prev_donation_shares
            else:
            ...
            if (
                abs(donation_shares_growth_rate - total_supply_growth_rate)
                ...
                donation_shares_used_delta = max(
                    0, expected_donation_shares - donation_shares
                )

                # Update normalized value: decrease proportionally to usage
                if (
                    prev_donation_shares_normalized is not None
                    and prev_donation_shares > 0
                ):
                    # Calculate usage ratio relative to previous donation_shares
                    usage_ratio = (
                        donation_shares_used_delta / prev_donation_shares
                        if prev_donation_shares > 0
                        else 0
                    )
        ...
        if donation_shares_used_delta is not None and donation_shares_used_delta > 0:
            ...
            used_ratio = (
                donation_shares_used_delta / total_supply if total_supply > 0 else 0
            )
...
if not last_prices_df.empty:
    last_prices_df = last_prices_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
if not price_scale_df.empty:
    price_scale_df = price_scale_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
if not price_oracle_df.empty:
    price_oracle_df = price_oracle_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
if not donation_shares_df.empty:
    donation_shares_df = donation_shares_df.drop_duplicates(
        subset=["timestamp"]
    ).sort_values("timestamp")
if not delta_price_df.empty:
    delta_price_df = delta_price_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
if not balance_df.empty:
    balance_df = balance_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
if not donation_releases_df.empty:
    donation_releases_df = donation_releases_df.drop_duplicates(
        subset=["release_time"]
    ).sort_values("timestamp")
if not donation_shares_usd_df.empty:
    donation_shares_usd_df = donation_shares_usd_df.drop_duplicates(
        subset=["timestamp"]
    ).sort_values("timestamp")
if not virtual_price_df.empty:
    virtual_price_df = virtual_price_df.drop_duplicates(
        subset=["timestamp"]
    ).sort_values("timestamp")
if not total_supply_df.empty:
    total_supply_df = total_supply_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
if not xcp_profit_df.empty:
    xcp_profit_df = xcp_profit_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
...
if not donation_shares_df.empty:
    ...
    donation_shares_df["delta"] = donation_shares_df["delta"].fillna(
        0
    )  # First value has no previous, set to 0
    ...
    balance_0_df = (
        balance_df[["timestamp", "balance_0"]].copy()
        if not balance_df.empty
        else pd.DataFrame()
    )
    balance_1_df = (
        balance_df[["timestamp", "balance_1"]].copy()
        if not balance_df.empty
        else pd.DataFrame()
    )
    ...
    donation_shares_df["delta_usd"] = donation_shares_df.apply(
        ...
        if (
            row["delta_filtered"] != 0
            and pd.notna(row["totalSupply"])
            and row["totalSupply"] > 0
            and pd.notna(row["balance_0"])
            and pd.notna(row["balance_1"])
    ...
    donation_shares_df["delta_usd_ma"] = (
        donation_shares_df["delta_usd"].rolling(window="2h", min_periods=1).mean()
    )
    ...
    if not positive_deltas.empty:
        for _idx, row in positive_deltas.iterrows():
            ...
            if (
                ...
                and total_supply > 0
                and balance_0 is not None
                and balance_1 is not None
                and last_price is not None
                and pd.notna(total_supply)
                and pd.notna(balance_0)
                and pd.notna(balance_1)
...
def create_refuel_chart(
    ...
    if not last_prices_df.empty:
        ax0.plot(
            last_prices_df["timestamp"],
            last_prices_df["last_price"],
            color=BLUE,
            label="spot price",
            linestyle="None",
            marker=MAKER,
    ...
    if not price_scale_df.empty:
        ax0.plot(
            price_scale_df["timestamp"],
            price_scale_df["price_scale"],
            color=GREEN,
            label="price scale",
            linestyle="None",
            marker=MAKER,
    ...
    if not last_prices_df.empty:
        ...
        last_prices_df_copy["price_change_pct"] = (
            last_prices_df_copy["last_price"] / max_price
        ) * 100
    ...
    ax0.set_title(
        f"{name}: Spot and Scale Prices Over Time", fontsize=14, fontweight="bold"
    )
    ...
    if not xcp_profit_df.empty:
        ax1.plot(
            xcp_profit_df["timestamp"],
            xcp_profit_df["xcp_profit"],
            "green",
            linestyle="None",
            marker=MAKER,
            markersize=MARKER_SIZE,
    ...
    if not virtual_price_df.empty:
        ...
        if not changed_data.empty:
            ax1.plot(
                changed_data["timestamp"],
                changed_data["virtual_price"],
                "red",
                marker=MAKER,
                markersize=MARKER_SIZE,
                linestyle="None",
                label="virtual_price / 2",
                alpha=0.7,
            )

    ax1.set_ylabel("Value", fontsize=12)
    ax1.set_title(
        f"{name}: xcp_profit and virtual_price / 2", fontsize=14, fontweight="bold"
    )
    ...
    if not donation_shares_df.empty and "delta_filtered" in donation_shares_df.columns:
        filtered_data = donation_shares_df[
            donation_shares_df["delta_filtered"] != 0
        ].copy()
        if not filtered_data.empty:
            ...
            if "delta_usd" in filtered_data.columns:
                for _idx, row in filtered_data.iterrows():
        ...
        if (
            not donation_shares_df.empty
            and "donation_shares" in donation_shares_df.columns
        ):
            ax3_twin_normalized = ax3.twinx()
            ax3_twin_normalized.spines["right"].set_position(("outward", 60))
            ax3_twin_normalized.plot(
                donation_shares_df["timestamp"],
        ...
        if "delta_usd_ma" in donation_shares_df.columns:
            ...
            ax3_twin_delta.plot(
                ...
                donation_shares_df["delta_usd_ma"],
                "orange",
                label="2h MA USD Spend",
                linestyle="None",
                marker=MAKER,
                markersize=MARKER_SIZE,
                alpha=0.8,
            )
            ax3_twin_delta.set_ylabel(
                "2h Moving Average USD Spend", fontsize=12, color="orange"
            )
            ax3_twin_delta.tick_params(axis="y", labelcolor="orange")

        ax3.set_ylabel("refuel in shares", fontsize=12)
        ax3.set_title(
            f"{name}: refuel shares used to rebalance", fontsize=12, fontweight="bold"
        )

        # Chart 3 legend
        handles3, labels3 = ax3.get_legend_handles_labels()
        if ax3_twin_normalized is not None:
            handles_twin_norm, labels_twin_norm = (
                ax3_twin_normalized.get_legend_handles_labels()
            )
    ...
    if not delta_price_df.empty:
        ...
        if not delta_usd_data.empty:
            ax4.plot(
                delta_usd_data["timestamp"],
                delta_usd_data["delta_usd"],
                "c",
                label="Delta (USD)",
                linestyle="None",
                marker=MAKER,
        ...
        if not delta_percent_data.empty:
            ...
            ax4_twin.plot(
                delta_percent_data["timestamp"],
                delta_percent_data["delta_percent"],
                "m",
                label="Delta (%)",
                linestyle="None",
                marker=MAKER,
        ...
        ax4.set_title(
            "Price Delta: last_prices() - price_scale()", fontsize=12, fontweight="bold"
        )
    ...
    if donation_reset_timestamps:
        for reset_time in donation_reset_timestamps:
            ax2.axvline(
                x=reset_time, color="black", linestyle="-", linewidth=1, alpha=0.7
            )
    ...
    if plot_description:
        print(
            f"{plot_description} saved to: {output_path} ({figure_width_pixels:.0f} x {int((FIGURE_HEIGHT_CM / 2.54) * _INTERNAL_DPI)} px)"
        )
    else:
        print(
            f"Chart saved to: {output_path} ({figure_width_pixels:.0f} x {int((FIGURE_HEIGHT_CM / 2.54) * _INTERNAL_DPI)} px)"
        )
...
dataframes_for_dimension_calc = [
    ...
    delta_price_df,
    balance_df,
    donation_shares_usd_df,
    virtual_price_df,
    total_supply_df,
]
figure_width_cm, figure_width_pixels, time_info = calculate_figure_dimensions(
    dataframes_for_dimension_calc
)
print(
    f"Full plot: {time_info}, width: {figure_width_pixels:.0f} px ({figure_width_cm:.1f} cm)"
)
...
output_path = (
    plot_dir / f"{name.replace('/', '_').replace(' ', '')}_refuel_analysis_all.png"
)
...
if all_timestamps:
    ...
    last_prices_df_window = (
        last_prices_df[last_prices_df["timestamp"] >= min_time_window].copy()
        if not last_prices_df.empty
        else pd.DataFrame()
    )
    price_scale_df_window = (
        price_scale_df[price_scale_df["timestamp"] >= min_time_window].copy()
        if not price_scale_df.empty
        else pd.DataFrame()
    )
    xcp_profit_df_window = (
        xcp_profit_df[xcp_profit_df["timestamp"] >= min_time_window].copy()
        if not xcp_profit_df.empty
        else pd.DataFrame()
    )
    virtual_price_df_window = (
        virtual_price_df[virtual_price_df["timestamp"] >= min_time_window].copy()
        if not virtual_price_df.empty
        else pd.DataFrame()
    )
    donation_shares_df_window = (
        donation_shares_df[donation_shares_df["timestamp"] >= min_time_window].copy()
        if not donation_shares_df.empty
        else pd.DataFrame()
    )
    delta_price_df_window = (
        delta_price_df[delta_price_df["timestamp"] >= min_time_window].copy()
        if not delta_price_df.empty
        else pd.DataFrame()
    )

    # Filter reset timestamps to time window
    donation_reset_timestamps_window = [
        ts for ts in donation_reset_timestamps if ts >= min_time_window
    ]
    ...
    figure_width_cm_window, figure_width_pixels_window, time_info_window = (
        calculate_figure_dimensions(dataframes_window)
    )
    print(
    ...
    output_path_window = (
        plot_dir
        / f"{name.replace('/', '_').replace(' ', '')}_refuel_analysis_{time_window_days}d.png"
    )
...
if not donation_shares_df.empty:
    print("\nRefule Shares (raw):")
    print(
        f"  Total Shares (latest): {donation_shares_df['donation_shares'].iloc[-1]:.6f}"
    )
...
if not donation_releases_df.empty:
    print("\nRefule Releases:")
    print(
        f"  Number of unique releases detected: {len(donation_releases_df['release_time'].unique())}"
    )
