...
def print_summary(df):
    ...
    if "epoch" in df.columns:
        ...
        min_time = datetime.fromtimestamp(min_epoch, UTC).strftime(
            "%Y-%m-%d %H:%M:%S UTC"
        )
        max_time = datetime.fromtimestamp(max_epoch, UTC).strftime(
            "%Y-%m-%d %H:%M:%S UTC"
        )
...
Examples:
  ...
  %(prog)s data/base/0xF30fcb00b7C3d2f6e12043157011bea7f848049D.parquet --export output.json --nested
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of rows to show from the beginning (default: 10)",
    )
    ...
    parser.add_argument(
        "--export", type=str, help="Export to file (CSV or JSON based on extension)"
    )
    parser.add_argument(
        "--nested",
        action="store_true",
        help="Export to nested JSON format (original structure)",
    )
    ...
    if args.export:
        ...
        if export_path.suffix == ".json":
            ...
            print(
                f"Warning: Unknown export format '{export_path.suffix}', defaulting to CSV"
            )
            filtered_df.to_csv(export_path, index=False)
