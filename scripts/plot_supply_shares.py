...
for _block_number, block_data in sorted(data.items(), key=lambda x: int(x[0])):
    ...
    if "user_supply" in block_data:
        user_supply_data.append(
            {"timestamp": timestamp, "user_supply": block_data["user_supply"]["value"]}
        )
    ...
    if "balances(0)" in block_data:
        balance_0_data.append(
            {"timestamp": timestamp, "balance_0": block_data["balances(0)"]["value"]}
        )

    # Extract balances(1) - WETH
    if "balances(1)" in block_data:
        balance_1_data.append(
            {"timestamp": timestamp, "balance_1": block_data["balances(1)"]["value"]}
        )
    ...
    if "fee" in block_data:
        fee_data.append({"timestamp": timestamp, "fee": block_data["fee"]["value"]})
...
if not user_supply_df.empty:
    user_supply_df = user_supply_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
...
if not balance_0_df.empty:
    balance_0_df = balance_0_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
if not balance_1_df.empty:
    balance_1_df = balance_1_df.drop_duplicates(subset=["timestamp"]).sort_values(
        "timestamp"
    )
...
if not balance_0_df.empty and not balance_1_df.empty and not last_prices_df.empty:
    ...
    balance_usd_df = pd.merge(
        balance_0_df, balance_1_df, on="timestamp", how="outer", suffixes=("", "_1")
    )
    balance_usd_df = pd.merge(
        balance_usd_df,
        last_prices_df,
    ...
    balance_usd_df["balance_1_usd"] = balance_usd_df["balance_1"].fillna(
        0
    ) * balance_usd_df["last_price"].fillna(0)
    balance_usd_df["total_usd"] = (
        balance_usd_df["balance_0_usd"] + balance_usd_df["balance_1_usd"]
    )

    # Calculate ratio (balance_0_usd / balance_1_usd or balance_0_usd / total_usd)
    balance_usd_df["ratio_0_to_1"] = balance_usd_df.apply(
        lambda row: row["balance_0_usd"] / row["balance_1_usd"]
        if row["balance_1_usd"] > 0
        else 0,
        axis=1,
    )
    balance_usd_df["ratio_0_to_total"] = balance_usd_df.apply(
        lambda row: row["balance_0_usd"] / row["total_usd"] * 100
        if row["total_usd"] > 0
        else 0,
        axis=1,
    )
    balance_usd_df["ratio_1_to_total"] = balance_usd_df.apply(
        lambda row: row["balance_1_usd"] / row["total_usd"] * 100
        if row["total_usd"] > 0
        else 0,
        axis=1,
    )
...
if all_timestamps:
    ...
    print(
        f"Full plot: {time_range_days:.2f} days, width: {figure_width_pixels:.0f} px ({figure_width_cm:.1f} cm)"
    )
...
if not donation_shares_df.empty:
    ...
    donation_shares_df["delta_usd_4h_ma"] = (
        donation_shares_df["delta_usd"].rolling(window="4h", min_periods=1).mean()
    )
...
if not donation_shares_df.empty:
    ax1.plot(
        ...
        "purple",
        linestyle="None",
        label="donation_shares",
        marker=MAKER,
        markersize=MARKER_SIZE,
    )
ax1.set_ylabel("donation_shares (absolute)", fontsize=12)
ax1.set_title(
    f"{name}: donation_shares Over Time (Absolute)", fontsize=14, fontweight="bold"
)
ax1.legend(
    bbox_to_anchor=(1.0, 1.15), loc="upper right", fontsize=10, ncol=1, frameon=False
)
...
if not donation_shares_df.empty and "delta_filtered" in donation_shares_df.columns:
    ...
    if not filtered_data.empty:
        ...
        ax2.bar(
            ...
            width=bar_width,
            color="purple",
            label="donation_shares delta (filtered: only negative)",
            align="center",
            edgecolor="purple",
            linewidth=0.1,
        )
...
if not balance_usd_df.empty:
    ...
    ax10.fill_between(
        ...
        alpha=0.7,
        color="cyan",
        label="USDC % of Total USD",
    )
    ax10.fill_between(
        timestamps_ratio,
        ratio_0,
        [r0 + r1 for r0, r1 in zip(ratio_0, ratio_1, strict=False)],
        alpha=0.7,
        color="orange",
        label="WETH % of Total USD",
    )
    ...
    ax10_twin.legend(
        bbox_to_anchor=(1.0, 1.15), loc="upper left", fontsize=10, ncol=1, frameon=False
    )

    ax10.set_ylabel("Percentage of Total USD (%)", fontsize=12)
    ax10.set_title(
        f"{name}: USD Value Distribution and Ratio Over Time",
        fontsize=14,
        fontweight="bold",
    )
    ax10.legend(
...
plt.suptitle(
    f"Supply and Shares Analysis: {name} (Data from {date_str})", fontsize=12, y=0.995
)
...
output_path = (
    plot_dir
    / f"{name.replace('/', '_').replace(' ', '')}_secondary_refuel_analysis.png"
)
plt.savefig(output_path, dpi=_INTERNAL_DPI, bbox_inches=None)
print(
    f"Chart saved to: {output_path} ({figure_width_pixels:.0f} x {int((FIGURE_HEIGHT_CM * 3 / 2.54) * _INTERNAL_DPI)} px)"
)
...
if not balance_usd_df.empty:
    ...
    print(
        f"  balance_0 (USDC) USD - Mean: ${balance_usd_df['balance_0_usd'].mean():.2f}"
    )
    print(
        f"  balance_0 (USDC) USD - Latest: ${balance_usd_df['balance_0_usd'].iloc[-1]:.2f}"
    )
    ...
    print(
        f"  balance_1 (WETH) USD - Mean: ${balance_usd_df['balance_1_usd'].mean():.2f}"
    )
    print(
        f"  balance_1 (WETH) USD - Latest: ${balance_usd_df['balance_1_usd'].iloc[-1]:.2f}"
    )
