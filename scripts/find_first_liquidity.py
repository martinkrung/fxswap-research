...
def main():
    ...
    for idx in indices:
        pool = config[idx]
        if "first_block" in pool and pool["first_block"] is not None:
            continue
        ...
        current_first = pool.get("first_block")
        ...
        if first_liq_block:
            if current_first != first_liq_block:
                logging.info(f"Updating first_block: {current_first} -> {first_liq_block}")
                pool["first_block"] = first_liq_block
                updated = True
            else:
                logging.info(f"first_block is already correct: {first_liq_block}")
